	    ...
	    for(int index1 = 0; index1<list.size()-1;index1++){
	        if(list.get(index1).getTimeSpan().end > list.get(index1+1).getTimeSpan().start){
	            ...
	            for(int j = 0; j< correct.getPointCount();j++){
	                /*System.err.format("replace time %f with %f int point %d\n",
	                       list.get(i).getPoint(j).get(ChannelName.T),
	                       correct.getPoint(j).get(ChannelName.T),
	                       j );*/
	...
	private boolean repairCanvasTraceFormat() throws InkMLComplianceException{
		...
		if(doc.getInk().getDefinitions().containsKey("inkAnnoCanvasFormat")){
			...
			if (f.getChannelCount() < 4){
				InkChannel.ChannelName[] names = {InkChannel.ChannelName.X,InkChannel.ChannelName.Y,InkChannel.ChannelName.T,InkChannel.ChannelName.F};
				for(InkChannel.ChannelName name : names){
					if(!f.containsChannel(name)){
						if(name == InkChannel.ChannelName.F){
