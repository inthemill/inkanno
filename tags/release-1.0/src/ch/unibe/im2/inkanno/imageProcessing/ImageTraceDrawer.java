...
public class ImageTraceDrawer extends TraceDrawVisitor {
	public void visitHook(InkTraceViewContainer container) {
		if(!Config.getMain().getB("no-marking") 
				|| (!(container.containsAnnotation(InkAnnoAnnotationStructure.TYPE) 
						&& container.getAnnotation(InkAnnoAnnotationStructure.TYPE).equals("Marking")))){
